...
function LayerStack({
  ...
  const maxScroll = React.useMemo(() =>
    Math.max(0, cards.length * (effectiveCardWidth + cardGap) -
      cardGap -
      effectiveCardWidth), [cards.length, effectiveCardWidth, cardGap]);

  const updateScrollX = React.useCallback((px) => {
  ...
  return (
    <div ref={ref} className={cn('w-full select-none', className)}>
      ...
      <div className='mt-4 flex flex-col items-center gap-2'>
        ...
        <p
          className='text-[10px] font-medium tracking-[0.14em] uppercase text-muted-foreground'>
