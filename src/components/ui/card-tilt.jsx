...
const CardTiltContext = React.createContext(null);
...
const CardTilt = React.forwardRef((
  ...
  const rotateX = useTransform(mouseYSpring, [-0.5, 0.5], tiltReverse
    ? [tiltMaxAngle, -tiltMaxAngle]
    : [-tiltMaxAngle, tiltMaxAngle]);
  const rotateY = useTransform(mouseXSpring, [-0.5, 0.5], tiltReverse
    ? [-tiltMaxAngle, tiltMaxAngle]
    : [tiltMaxAngle, -tiltMaxAngle]);
  ...
  return (
    <CardTiltContext.Provider value={{ rotateX, rotateY, scale: scaleValue }}>
      <div
        ...
        {...props}>
        <div
          className='absolute inset-0 rounded-2xl border-2 border-dashed border-slate-300' />
        {children}
      </div>
...
const CardTiltContent = React.forwardRef(({ children, className, ...props }, ref) => {
